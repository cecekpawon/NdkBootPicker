  ...
  )
{
  ...
  for (Y = 0; Y < Height; ++Y) {
    ...
    for (X = 0; X < Width; ++X) {
      ...
      if (TopAlpha == 255) {
        TempColor         = Base == 0
                              ? TopPtr->Blue + (TopPtr->Blue * ColorDiff / 255)
                              : MIN (TopPtr->Blue + (TopPtr->Blue * ColorDiff / 255), Base);
        CompPtr->Blue     = (UINT8) TempColor;

        TempColor         = Base == 0
                              ? TopPtr->Green + (TopPtr->Green * ColorDiff / 255)
                              : MIN (TopPtr->Green + (TopPtr->Green * ColorDiff / 255), Base);
        CompPtr->Green    = (UINT8) TempColor;

        TempColor         = Base == 0
                              ? TopPtr->Red + (TopPtr->Red * ColorDiff / 255)
                              : MIN (TopPtr->Red + (TopPtr->Red * ColorDiff / 255), Base);
        ...
        CompPtr->Blue     = (UINT8) ((Base == 0
                                        ? TopPtr->Blue + (TopPtr->Blue * ColorDiff / 255)
                                        : MIN (TopPtr->Blue + (TopPtr->Blue * ColorDiff / 255), Base) * TopAlpha + CompPtr->Blue * TempAlpha) / Alpha);
        CompPtr->Green    = (UINT8) ((Base == 0
                                        ? TopPtr->Green + (TopPtr->Green * ColorDiff / 255)
                                        : MIN (TopPtr->Green + (TopPtr->Green * ColorDiff / 255), Base) * TopAlpha + CompPtr->Green * TempAlpha) / Alpha);
        CompPtr->Red      = (UINT8) ((Base == 0
                                        ? TopPtr->Red + (TopPtr->Red * ColorDiff / 255)
                                        : MIN (TopPtr->Red + (TopPtr->Red * ColorDiff / 255), Base) * TopAlpha + CompPtr->Red * TempAlpha) / Alpha);
        CompPtr->Reserved = (UINT8) (Alpha / 255);
      }
  ...
  )
{
  ...
  NewImage = CreateImage ((UINT16) Width, (UINT16) Height, IsAlpha);
  if (NewImage == NULL) {
    return NULL;
  }
  ...
  )
{
  ...
  if (Ratio == 16) {
    ...
    NewImage = CreateImage ((UINT16) NewW, (UINT16) NewH, OldImage->IsAlpha);
    if (NewImage == NULL) {
      return NULL;
    }
